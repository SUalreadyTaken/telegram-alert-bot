...
public class CheckPrice {
  ...
  public void checkWatchlist(double currentPrice) {
    ...
    if (!dbPricesToRemove.isEmpty()) {
      for (Map.Entry<Double, List<Integer>> entry : dbPricesToRemove.entrySet()) {
        try {
          dbCommandsQueue.getDbCommandsQueue()
              .put(new DBCommand(DBCommandType.REMOVECHATIDS, entry.getKey(), entry.getValue()));
        } catch (InterruptedException e) {
          e.printStackTrace();
        }
      }
  ...
  /**
   * Gets bitmex btc price. Returns 0 if response code is 429 meaning request
   * limit reached (will sleep for 60sec) and if api returns 502 which happens
   * from time to time.
   *
   * @return btc price
   */
  private double getPrice() {
    ...
    try {
      ...
      // "If you are limited, you will receive a 429 response and an additional
      // header"
      // Sleep 60sec

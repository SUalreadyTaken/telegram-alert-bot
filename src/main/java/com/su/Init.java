...
public class Init {
	...
	public Init(AlertBot alertBot, PriceDataRepository priceDataRepository, PriceWatchList priceWatchList, IdleService idleService,
				MessageToSend messageToSend, DBCommandsQueue dbCommandsQueue, PriceDataService priceDataService) {
	...
	private void start() {
		if (!switchApp || !idleService.getAlternativeBoolean()) {
			...
			try {
				...
				Thread dbControllerThread = new Thread(new DBController(dbCommandsQueue.getDbCommandsQueue(), priceDataService));
